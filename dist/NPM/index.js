...
function TABLE_DEFINITION_exec_groups (_        )  {                                                                                                         
	...
	let tagInner         = '';
	if ( _.startsWith('(') ) { ( { 1: tagInner, 2: _ } = TAG_REST.exec(_) || throws(SyntaxError(where())) ); }
	_.startsWith(']') || throws(SyntaxError(where()));
	const $$asArrayItem$_          = _.charAt(1)===']';
	_ = _.slice($$asArrayItem$_ ? 2 : 1).replace(PRE_WHITESPACE, '');
	let tagOuter         = '';
	if ( _.startsWith('(') ) { ( { 1: tagOuter, 2: _ } = TAG_REST.exec(_) || throws(SyntaxError(where())) ); }
...
const Float = (literal        )         => {
	if ( FLOAT.test(literal) && FLOAT_NOT_INTEGER.test(literal) ) {
		if ( sFloat ) { return +literal.replace(UNDERSCORES, ''); }
		else {
...
function prepareTable (table        , keys          )         {
	...
	while ( index<length ) {
		...
		if ( key in table ) {
			...
			else { throws(Error('Trying to define table through non-Table value at '+where())); }
		}
		else {
...
function push (lastInlineTable_array                , lineRest        )         {
	...
	if ( literal==='true' ) { asBooleans(lastInlineTable_array).push(true); }
	else if ( literal==='false' ) { asBooleans(lastInlineTable_array).push(false); }
	...
	else if ( enableNull && literal==='null' ) { asNulls(lastInlineTable_array).push(null); }
	else { asIntegers(lastInlineTable_array).push(IntegerDepends(literal)); }
	return lineRest;
}
