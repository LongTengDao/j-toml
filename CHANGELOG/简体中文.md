...
1.  0.  0.  +   新增功能：支持 TOML v1.0.0
    ...
    23. 0.  *   基于 TypeScript 4.5 新增的 `.d.mts` 扩展名和 `package.json` 的 `exports` 字段中的 `types` 类别，为 NPM 版本中的 ESM 模式提供了专属的类型声明文件 `index.d.mts`
            *   兼容性：确保当 Node.js 引擎改变其内部工作方式时，加速的代码能够回退到常规写法
            *   兼容性：去除正则中的前文断言语法，以兼容 Safari
    ...
    33. 0.  *   对传给 `TOML.parse` 数组（通常来自 `TOML.stringify` 没有设置 `选项.newline` 时的输出结果）的行为进行更加友好的错误提示
            *   优化：避免 `TOML.parse` 对传入的 `Buffer`、`Uint8Array` 或 `ArrayBuffer` 进行两次编码检测
            *   优化：`TOML.stringify` 的 `-NaN` 处理
        1.  *   故障修复：兼容某些宿主（如旧 Edge 浏览器）的 `NaN` 实现（始于 1.32.0）
        2.  *   故障修复：改正闰年判定机制（始于 1.5.2）
        3.  *   故障修复：`TOML.OffsetDateTime` 的闰秒支持（始于 0.5.*）
            *   故障修复：1970 年之前的 `TOML.OffsetDateTime` 实现（始于 0.5.*）
    34. 0.  +   新增功能：新增了 `TOML.Keys` 与 `选项.keys`，用于避免解析过程中的哈希攻击
            *   遵循文档：不再只获取传递给 `TOML.parse` 的 `选项` 对象自身的属性（文档中从未提及的功能）
            *   对 `TOML.parse` 参数错误更加友好的错误提示
    35. 0.  *   优化：`TOML.parse` `超级选项.tag` 的内部实现从数组改为链表
        1.  *   修复类型文件（始于 1.34.0）
        2.  *   修复类型文件（始于 1.35.1）
        3.  *   故障修复：修复 1970 年前不包含小数秒的坐标日期时刻的解析崩溃问题（始于 1.33.3）
            *   兼容性：去除了 `?.`、`??` 语法
            *   兼容性：去除了 `bigint` 语法（但在未提供 `BigInt` 的运行环境下 `TOML.parse` 时，必须将其关联选项明确设置为 `false`，否则将报错）
    36. 0.  +   新增功能：为日期时刻实现增加了不同于 `Object` 的 `Symbol.toStringTag`
    37. 0.  +   新增功能：为四种类型的日期时刻分配不同的 `Symbol.toStringTag`
    38. 0.  +   对 0.4 及之前模式中的正负无穷不再报错，因为它们可能通过超出取值范围得到，而本库对于低版本 TOML 规范的支持策略是以不造成不必要的错误为一贯的准则的
